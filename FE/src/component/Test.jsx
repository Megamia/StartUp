...
function Row({ row ,fetchData}) {
  ...
  const  delData=async(id,name)=>{
    if(window.confirm(`Bạn chắc chắn muốn xóa dữ liệu có tên ${name}?`)){
      try {
        const response = await axios.delete(`${process.env.REACT_APP_API_URL}/api/deleteData/${id}`);
  ...
  return (
    <>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell align="center">{row.profit !=null?row.profit:0 }</TableCell>
...
export default function CollapsibleTable() {
  ...
  const addNewData = async () => {
    if (showInput) {  
      try {
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/addData`,
          {
            name: dataName,
  ...
  const totalPurchase = Array.isArray(data) && data.length > 0
    ? data.reduce((acc, row) => acc + row.purchase, 0)
    : 0;

  const totalSelling = Array.isArray(data) && data.length > 0
    ? data.reduce((acc, row) => acc + row.selling, 0)
    : 0;
    
  const totalProfit = totalSelling-totalPurchase;
  const filteredData = data.filter((row) => {
    const matchesName = row.name.toLowerCase().includes(filterText.toLowerCase());
  ...
  return (
    <div className="flex flex-col flex-1 p-[20px] gap-[20px]">
      <div className='flex w-[45%] justify-between' >
        <input
          type="text"
          className="border-[1px] border-black w-[200px] placeholder:pl-[5px]"
          value={filterText}
          onChange={(e) => setFilterText(e.target.value)} 
          placeholder="Filter by name or purchase"
        />
        <button onClick={addNewData} className='bg-[#48B02C] px-[10px] text-white'>
          Thêm mới
        </button>
      </div>
      {showInput&&(
            ...
            <input
              type="text"
              className="border-[1px] border-black w-[200px]"
              value={dataName}
        ...
        <Table aria-label="collapsible table">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center">
              <span className="flex items-center gap-2 cursor-pointer hover:text-gray-400" onClick={toggleSortOrder}>
                  {sortOrder === 'asc' ? (
                    <>
          ...
          <TableBody>
              {filteredData.length > 0 ? (
                sortData(filteredData).map((row, index) => <Row key={index} row={row} fetchData={fetchData} />)
              ) : (
                <TableRow>
